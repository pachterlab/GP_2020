function P=gg_200325_analyt_geom_tdep_vec_31(Kini,b,g,M,N,T,marg,N_approx_taylor,N_approx_laurent)
...
% I___ = NaN(M*N,1);
alpha=((1+sqrt(3))/2)/b;
% [roots1,xt1,s01]=root_ID(g,TH1,TH2,alpha,I,J);
[roots,xt,s0]=root_ID(g,TH1,TH2,alpha,I,J);
% f=find(any(roots-roots1>1e-4,2));
% length(f)
% if length(f)>10
% s01(f(1:10),:)
% s0(f(1:10),:)
% end
% root_viz(b,g,U,V,alpha,M,N,8,8,roots)
...
function I = numint(b,g,u,v,t1,t2)
f = @(x) (b*exp(-g*x).*(u+g*v*x))./(1-b*exp(-g*x).*(u+g*v*x));
I = integral(f,t1,t2);
% I=exp(KINI*INTVAL);
return

function root_viz(b,g,U,V,alpha,M,N,Mmax,Nmax,s)
t=5;
n_t = 1e4;
U=reshape(U,[M,N]);
V=reshape(V,[M,N]);
...
for i = 1:min(Mmax,M)
    for j = 1:min(Nmax,N)
        ...
        v = V(i,j);
        figure(3)
        subplot(Mmax,Nmax,Nmax*(i-1)+j);hold on;
        t_ =linspace(0,t,n_t);
        fval = abs(b*(u+g*v*t_).*exp(-g*t_));
        plot(t_,fval,'k-');hold on;
        set(gca,'xtick',[],'ytick',[]);

        plot([0 t],alpha*[1 1],'b-');

        yl=ylim;
        for SIND = 1:3
            plot(s(i,j,SIND)*[1 1],yl,'r--')
        end
    end
end
...
function integral_viz(b,g,u,v,alpha,i,j,Mmax,Nmax,s)
t=5;
if i<=Mmax && j<=Nmax
    figure(3)
    subplot(Mmax,Nmax,Nmax*(i-1)+j);hold on;
    t_ =linspace(0,t);
    U = abs(b*(u+g*v*t_).*exp(-g*t_));
    plot(t_,U,'k-');hold on;
    set(gca,'xtick',[],'ytick',[]);
    ...
    yl=ylim;
    for SIND = 1:3
        plot(s(SIND)*[1 1],yl,'r--')
    end
end
return

function A = func_val(b,g,u,v,s)
A=b*exp(-g*s)*(u+g*v*s);
return

function A = func_abs_val(b,g,u,v,s)
A=abs(b*exp(-g*s).*(u+g*v*s));
return

function [s,r,s0]=root_ID_old(g,th1,th2,alpha,i,j)
n = length(th1);
...
function x=fp(x,b1,b2,func,tol)
return
...
function I = numint_power_series(g,u,v,t1,t2,k)
k=length(k);
n=length(u);
I=NaN(n,k);
for i = 1:k
    for j = 1:n
        f = @(x) (exp(-g*x).*(u(j)+g*v(j)*x)).^i;
        I(j,i) = integral(f,t1(j),t2(j));
    end
end
...
function I = numint_laurent_power_series(g,u,v,t1,t2,k)
I=NaN(1,k+1);
for i = 0:k
    f = @(x) (exp(-g*x).*(u+g*v*x)).^-i;
    I(i+1) = integral(f,t1,t2);
end
