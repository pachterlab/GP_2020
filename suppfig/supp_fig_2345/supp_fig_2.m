...
%     
    axes(ha1(DATANUM));
    [~,HH]=contourf(log10(F.KG_),log10(F.B_),reshape(DATA_TO_PLOT,F.n_kg,F.n_b),100);hold on;
    set(HH,'LineColor',LINECOL);
    scatter(log10(D.kpar(1)), log10(D.S(1)), 40,'r','filled');
    colormap(map);

    h=get(gca);
    set(gca,'xtick',[],'ytick',[],'ycolor','none','FontSize',12,'visible','off','box','on');
